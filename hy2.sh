...
clean_jump_iptables(){
    # 仅清理带注释 HY2_JUMP 的规则，防止误删用户原有 NAT/Docker 规则
...
EOF
    ...
    if systemctl is-active --quiet hysteria-server; then
        ...
        red "Hysteria 2 启动失败，请检查 systemctl status hysteria-server 查看具体日志。" && exit 1
    fi
...
menu() {
    clear
    echo "#############################################################"
    echo -e "#                  ${GREEN}Hysteria 2 管理脚本 (优化版)${PLAIN}             #"
    echo "#############################################################"
    echo ""
