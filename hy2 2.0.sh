...
# 动态识别官方服务名（hysteria-server@config 或 hysteria-server）
get_service_name(){
...
# 防火墙持久化保存（兼容 Debian/Ubuntu 与 RHEL 系列）
save_iptables(){
...
realip(){
    ...
    [[ -z $ip ]] && ip=$(curl -s4m8 ifconfig.me -k) || ip=$(curl -s6m8 ifconfig.me -k)
}
...
fix_permissions(){
    # 官方服务通常以 hysteria 用户运行，必须保证该用户对目录与证书私钥可读
...
menu() {
    clear
    echo "#############################################################"
    echo -e "#                  ${GREEN}Hysteria 2 管理脚本 (稳定版)${PLAIN}             #"
    echo "#############################################################"
    echo ""
